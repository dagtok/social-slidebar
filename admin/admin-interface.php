...
/* Admin Interface
/*-----------------------------------------------------------------------------------*/
define("THEME_ADMIN", "Social Slidebar");

$functions_path = THEME_ADMIN . '/';
function kbl_ssb_siteoptions_add_admin() {

    global $query_string;
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'kblssbsiteoptions' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			$options =  get_option('kbl_ssb_of_template'); 
			kbl_ssb_of_reset_options($options,'kblssbsiteoptions');
			header("Location: admin.php?page=siteoptions&reset=true");
			die;
		}
    }
		
    $kbl_ssb_page = add_theme_page('Social slidebar', 'Social slidebar', 'edit_theme_options', 'kblssbsiteoptions','kbl_ssb_siteoptions_options_page');
	add_action("admin_print_scripts-$kbl_ssb_page", 'kbl_ssb_of_load_only');
	add_action("admin_print_styles-$kbl_ssb_page",'kbl_ssb_of_style_only');
} 

add_action('admin_menu', 'kbl_ssb_siteoptions_add_admin');
...
function kbl_ssb_of_reset_options($options,$page = ''){
	...
	//When Theme Options page is reset - Add the kbl_ssb_of_options option
	if($page == 'kblssbsiteoptions'){
		$query_inner .= " OR option_name = 'kbl_ssb_of_options'";
	}
...
function kbl_ssb_siteoptions_options_page(){
    $options =  get_option('kbl_ssb_of_template');      
    $themename =  get_option('kbl_ssb_of_themename');
?>

<div class="wrap" id="truethemes_container">
  <div id="of-popup-save" class="of-save-popup">
    <div class="of-save-save">Social Feeds And Settings Updated</div>
  </div>
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset">Social Feeds And Settings Reseted</div>
  </div>
  <form action="" enctype="multipart/form-data" id="ofform">
    <div id="header">
      <div class="logo">
        <h2>Social Slidebar v0.0</h2>
      </div>
    ...
    <?php 
        ...
        $return = kbl_ssb_siteoptions_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
    ...
    <img style="display:none;" src="<?php echo plugins_url(); ?>/social-slidebar/admin/images/wpspin_light.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
...
function kbl_ssb_of_style_only() {
	wp_enqueue_style('admin-style', plugins_url().'/social-slidebar/admin/admin-style.css');
	wp_enqueue_style('color-picker', plugins_url().'/social-slidebar/admin/colorpicker.css');
	// $color = get_user_option('kbl_ssb_admin_color');
	// if ($color == "fresh")
	// {
	wp_enqueue_style('admin-style-grey', plugins_url().'/social-slidebar/admin/admin-style-grey.css');
	wp_enqueue_style('color-picker', plugins_url().'/social-slidebar/admin/colorpicker.css');
	// }
}
...
function kbl_ssb_of_load_only() {

	add_action('admin_head', 'kbl_ssb_of_admin_head');
	
	wp_enqueue_script('jquery-ui-core');
	wp_register_script('jquery-input-mask', plugins_url().'/social-slidebar/admin/js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('jquery-input-mask');
	wp_enqueue_script('color-picker', plugins_url().'/social-slidebar/admin/js/colorpicker.js', array('jquery'));
	wp_enqueue_script('ajaxupload', plugins_url().'/social-slidebar/admin/js/ajaxupload.js', array('jquery'));
	
	function kbl_ssb_of_admin_head() { 
	?>
...
<script type="text/javascript" language="javascript">
		...
		if (typeof AjaxUpload != 'function') { 
			...
			//Color Picker
			<?php $options = get_option('kbl_ssb_of_template');
  ...
  jQuery("a[title=submit]").click( function(){
    ...
    jQuery(this).parents("form").submit();
			...
			jQuery('#ofform').submit(function(){
					...
					var data = {
						<?php if(isset($_REQUEST['page']) && $_REQUEST['page'] == 'kblssbsiteoptions'){ ?>
						type: 'options',
						<?php } ?>
...
add_action('wp_ajax_of_ajax_post_action', 'kbl_ssb_of_ajax_callback');

function kbl_ssb_of_ajax_callback() {
...
function kbl_ssb_siteoptions_machine($options) {
	...
	foreach ($options as $value) {
		...
		case "multicheck":
			...
			$kbl_ssb_key = $value['id'] . '_' . $key;
			$saved_std = get_option($kbl_ssb_key);
			...
			elseif( $std == $key) {
			   $checked = 'checked="checked"';
				...
				$checked = '';                                                                                    }
			$output .= '<input type="checkbox" class="checkbox of-input" name="'. $kbl_ssb_key .'" id="'. $kbl_ssb_key .'" value="true" '. $checked .' /><label for="'. $kbl_ssb_key .'">'. $option .'</label><br />';
		...
		case "upload":
			...
			$output .= kbl_ssb_siteoptions_uploader_function($value['id'],$value['std'],null);
		...
		case "upload_min":
			...
			$output .= kbl_ssb_siteoptions_uploader_function($value['id'],$value['std'],'min');
...
function kbl_ssb_siteoptions_uploader_function($id,$std,$mod){
